...
const { TARGET_TYPES, _certRecipients, _SEUILS_ALERTE, _ordreHa, _haDelaySec } =
...
await t("le backup est servi avant le primaire", () => {
  const cible = (id, role, ordre) => ({ id, deploy_order: ordre, params: role ? { ha_role: role } : {} });
  const ordre = _ordreHa([
    cible(1, "primary", 0), cible(2, null, 5), cible(3, "backup", 9), cible(4, "primary", 0),
  ]).map(x => x.id);
  assert.strictEqual(ordre[0], 3, "le backup n'est pas servi en premier");
  assert.strictEqual(ordre[1], 2, "le n\u0153ud isol\u00e9 devrait passer avant les primaires");
  assert.deepStrictEqual(ordre.slice(2).sort(), [1, 4], "les primaires ne sont pas en dernier");
...
await t("d\u00e9lai avant le primaire : valeurs par d\u00e9faut et aberrantes", () => {
  const c = v => ({ params: v === undefined ? {} : { ha_delay_sec: v } });
  assert.strictEqual(_haDelaySec(c()), 120, "d\u00e9faut attendu \u00e0 120 s");
  assert.strictEqual(_haDelaySec(c("300")), 300);
  assert.strictEqual(_haDelaySec(c(0)), 0, "un d\u00e9lai nul doit rester nul");
  assert.strictEqual(_haDelaySec(c("abc")), 120, "valeur illisible : repli sur 120");
  assert.strictEqual(_haDelaySec(c(-5)), 120, "valeur n\u00e9gative : repli sur 120");
  assert.strictEqual(_haDelaySec(c(999999)), 86400, "d\u00e9lai born\u00e9 \u00e0 24 h");
  assert.strictEqual(_haDelaySec(undefined), 120);
});
await t("les deux types HA exposent le r\u00f4le et le d\u00e9lai", () => {
  for (const type of ["haproxy", "aloha"]) {
    ...
    assert.ok(cles.includes("ha_delay_sec"), type + " : champ ha_delay_sec absent");
    const role = d.fields.find(f => f.key === "ha_role");
    assert.strictEqual(role.type, "select");
    const valeurs = role.options.map(o => o[0]);
    assert.deepStrictEqual(valeurs, ["", "backup", "primary"]);
  }
});
